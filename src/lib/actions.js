...
export async function createProveedor(prevState, formData) {
  ...
  try {
    ...
    if (file.size > 0) {
      const imagen = await imgCreate(file);
      query =
        "insert into proveedores(nombre,descripcion,telefono,email,imagen) values (?, ?, ?, ?, ?)";
      results = await db.query(query, [
        nombre,
        descripcion,
        telefono,
      ...
      results = await db.query(query, [nombre, descripcion, telefono, email]);
    }
    // console.log(results);
...
export async function updateProveedor(prevState, formData) {
  ...
  try {
    ...
    if (file.size > 0) {
      const imagen = await imgCreate(file);
      results = await db.query(query, [
        { nombre, descripcion, telefono, email, imagen },
        id,
      ]);
    } else {
      results = await db.query(query, [
